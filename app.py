...
app = Flask(__name__)
app.config['SQLALCHEMY_DATABASE_URI'] = 'sqlite:///marketplace.db'  # Adjust the database URI as needed
db = SQLAlchemy(app)
...
class Supplier(db.Model):
    ...
    matched_product = db.Column(db.String(50), nullable=True)
...
def calculate_costs():
    ...
    for i in range(len(data) // 2):
        product_name = data[f'product_{i}']
        quantity = int(data[f'quantity_{i}'])

        # Assume a fixed unit price or fetch from the database
        unit_price = 100.0  # Adjust based on your logic
    ...
    return jsonify({'total_cost': total_cost, 'breakdown': products})

# Endpoint to recommend suppliers
@app.route('/recommend_suppliers', methods=['POST'])
def recommend_suppliers():
    data = request.form.to_dict()
    keyword = data.get('product_keyword', '')
    location = data.get('preferred_location', '')
    limit = int(data.get('limit', 10))

    # Querying suppliers based on keyword and location
    suppliers = Supplier.query.filter(
        (Supplier.matched_product.like(f'%{keyword}%')) |
        (Supplier.location.like(f'%{location}%'))
    ).limit(limit).all()
    ...
    results = [{
        ...
        'Location': s.location,
        'Price': s.price,
        'Matched Product': s.matched_product,
